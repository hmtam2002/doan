...
<?php if ($type == 'chi-nhanh') {
	$chi_nhanh = $d->rawQuery("SELECT name$lang,iframe,diachi from table_news where type='chi-nhanh' and find_in_set('hienthi',status) order by numb");
	if ($chi_nhanh) { ?>
		<div class="block-chi-nhanh">
				...
				<?php foreach ($chi_nhanh as $key => $value) { ?>
					<div class="item-toa-do <?php echo $key == 0 ? "active" : "" ?>">
						<div class="content_toado">
							<input type="hidden" value="<?php echo $func->match_iframe_src(htmlspecialchars_decode($value['iframe'])) ?>">
							<h2 class="d-block ten-chi-nhanh"><?php echo $value['name' . $lang] ?> </h2>
			...
			<div class="iframe_chinhanh">
				<iframe src="<?php echo $func->match_iframe_src(htmlspecialchars_decode($chi_nhanh[0]['iframe'])) ?>" frameborder="0"></iframe>
			</div>
		</div>
		<script>
			$(document).ready(function() {
				$("body").on("click", ".item-toa-do", function(event) {
					...
					$(".iframe_chinhanh iframe").attr("src", src);
				});
			});
		</script>
<?php }

...
$forder_arr=explode('/',$config['database']['url']);
$folder = $forder_arr[count($forder_arr)-2];
...
$table_all = array('table_counter','table_excel','table_gallery','table_lang','table_member','table_news','table_newsletter','table_news_cat','table_news_item','table_news_list','table_news_sub','table_order_detail',' table_permission','table_permission_group','table_product', 'table_product_brand', 'table_product_cat', ' table_product_item', 'table_product_list', 'table_product_sub', ' table_color', 'table_size', 'table_comment', '  table_comment_photo', 'table_comment_video', 'table_city', 'table_district', '  table_ward',  'table_pushonesignal','table_seo','table_tags', 'table_product_tags', 'table_seopage');
...
if($check==true) {  
  removeAllFile('upload/color');
  removeAllFile('upload/excel');
  removeAllFile('upload/file');
  removeAllFile('upload/filemanager');
  removeAllFile('upload/news');
  removeAllFile('upload/photo');
  removeAllFile('upload/product');
  removeAllFile('upload/seopage');
  removeAllFile('upload/sync');
  removeAllFile('upload/tags');
  removeAllFile('upload/temp');
  removeAllFile('upload/user');
  removeAllFile('upload/video');
}
...
if (file_exists($filename)) {
  ...
  $html='<a href="'.$name_file.'?act='.$act_upload.'">Upload Kỹ thuật</a>';
  $html.='<a href="'.$name_file.'?act='.$act_download.'" style="margin-left:25px">Download Source</a>';
  echo $html;
}
// /* 5. end-------------------------------------------------------*/

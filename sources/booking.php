<?php 
	...
	if(!empty($_POST['submit-booking']))
	{
        ...
        if(($scoreCaptcha >= 0.5 && $actionCaptcha == 'Booking') || $testCaptcha == true)
		{
            ...
            {
                $func->transfer("Đặt lịch hẹn thất bại. Vui lòng thử lại sau.", $_POST['url-current'], false);
            }
			...
			if($emailer->send("admin", $arrayEmail, $subject, $message, $file))
			{
				...
				if($emailer->send("customer", $arrayEmail, $subject, $message, $file)) $func->transfer("Đặt lịch hẹn thành công", $_POST['url-current']);
			}
			else $func->transfer("Đặt lịch hẹn thất bại 1. Vui lòng thử lại sau", $_POST['url-current'], false);
		}
		else
		{
			$func->transfer("Đặt lịch hẹn thất bại 2. Vui lòng thử lại sau", $_POST['url-current'], false);
		}
	}
?>